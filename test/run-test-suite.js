...
let groups = fs.readdirSync(path.join(__dirname, "test-suite", "groups"));
...
let datasets = JSON.parse(fs.readFileSync(path.join(__dirname, "test-suite", "datasets.json")).toString());
...
describe("JSONata Test Suite", () => {
    ...
    groups.forEach(group => {
        ...
        let cases = readJSON(path.join("test-suite", "groups"), group);
        describe("Group: " + group, () => {
            ...
            for (let i = 0; i < cases.length; i++) {
                ...
                it(testcase.expr, function() {
                    ...
                    if (expr) {
                        // Load the input data set (if there is one)
                        let dataset = testcase.dataset === -1 ? undefined : datasets[testcase.dataset];
